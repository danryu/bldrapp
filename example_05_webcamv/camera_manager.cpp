...
bool CameraManager::enumerateCameras() {
    ...
    for (GList* l = devices; l != nullptr; l = l->next) {
        ...
        // No need to check factory with gst_device_create_element just to validate source type,
        // the monitor filter "Video/Source" is sufficient for basic filtering.
        // We trust the monitor.
...
QStringList CameraManager::cameraNames() const {
    ...
    for (const auto& cam : cameras_) {
        names << QString::fromStdString(cam.displayName);
    }
    return names;
}
...
GstElement* CameraManager::createCurrentCameraElement() {
    if (currentCameraIndex_ < 0 || currentCameraIndex_ >= static_cast<int>(cameras_.size())) {
        return nullptr;
    }

    const CameraDevice& cam = cameras_[currentCameraIndex_];
    if (cam.device) {
        return gst_device_create_element(cam.device, nullptr);
    } 
