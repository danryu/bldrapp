...
auto jitsibin_pad_added_handler(GstElement* const /*jitsibin*/, GstPad* const pad, gpointer const data) -> void {
    ...
    unwrap_mut(osxvideosink, add_new_element_to_pipeine(self.pipeline, "osxvideosink"));
    g_object_set(&dec,
    ...
    ensure(gst_element_link_pads(&videoconvert, NULL, &osxvideosink, NULL) == TRUE);
    ensure(gst_element_sync_state_with_parent(&videoconvert) == TRUE);
    ensure(gst_element_sync_state_with_parent(&osxvideosink) == TRUE);
